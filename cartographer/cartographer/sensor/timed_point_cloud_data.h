...
// 单个激光传感器获得的点云数据格式 (base_link下)
struct TimedPointCloudData {
  common::Time time;
  Eigen::Vector3f origin; // 激光原点在base_link下的位置
  sensor::TimedPointCloud ranges; // base_link下
};

// 多激光传感器经过时间同步后得到点云数据格式 (base_link下)
struct TimedPointCloudOriginData {
  struct RangeMeasurement {
    Eigen::Vector4f point_time; // base_link下
    size_t origin_index;
  };
  common::Time time;
  std::vector<Eigen::Vector3f> origins; // base_link下
  std::vector<RangeMeasurement> ranges; // base_link下
};

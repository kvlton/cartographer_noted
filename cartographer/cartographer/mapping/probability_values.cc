...
float SlowValueToBoundedFloat(const uint16 value, const uint16 unknown_value,
  ...
  if (value == unknown_value) return unknown_result; // 未知的
...
///--------------------------计算hit_table_ 和 miss_table_------------------------------

/// 使用Odds, 来计算hit_table_ 和 miss_table_(unused)
std::vector<uint16> ComputeLookupTableToApplyOdds(const float odds) {
  std::vector<uint16> result;
  result.push_back(ProbabilityToValue(ProbabilityFromOdds(odds)) +
                   kUpdateMarker); // unkonw
...
/// 使用CorrespondenceCostOdds, 来计算hit_table_ 和 miss_table_
/// hit_table_和miss_table_最终保存的是CorrespondenceCostValue p(s=0)对应的Value
std::vector<uint16> ComputeLookupTableToApplyCorrespondenceCostOdds(
  ...
  for (int cell = 1; cell != 32768; ++cell) {
    result.push_back(
        ...
        kUpdateMarker); // 加一个偏移量, 表示已经进行了更新, 用来防止重复更新
  }
  return result;
}

...
void PoseExtrapolator::AddOdometryData(
  ...
  // 1.计算angular_velocity_from_odometry_
  ...
  // 2.计算linear_velocity_from_odometry_(odom坐标系下)
...
void PoseExtrapolator::AdvanceImuTracker(const common::Time time,
  ...
  // 1.无IMU消息, 则根据pose或者odometry对imu_tracker进行预测更新
  ...
  // 2.有IMU消息, 利用IMU数据对imu_tracker进行更新, 再进行预测
  // 2.1 无IMU数据的时间段
...
// 相对于上个pose的rotation(使用imu_tracker进行外推)
...
// 相对于上个pose的translation(使用odometry进行外推)

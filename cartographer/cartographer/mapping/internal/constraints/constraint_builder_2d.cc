...
void ConstraintBuilder2D::MaybeAddConstraint(
  ...
  // 1.为submap构造FastCorrelativeScanMatcher2D (需要一些时间, 作为task加入线程池)
  const auto* scan_matcher =
      DispatchScanMatcherConstruction(submap_id, submap->grid());
  auto constraint_task = common::make_unique<common::Task>();
  // 2.node和submap闭环匹配构造约束(作为task加入线程池)
  constraint_task->SetWorkItem([=]() EXCLUDES(mutex_) {
    ComputeConstraint(submap_id, submap, node_id, false, /* match_full_submap */
                      constant_data, initial_relative_pose, *scan_matcher,
                      constraint);
  });
  // node和submap的匹配, 依赖于submap的FastCorrelativeScanMatcher2D构造
...
void ConstraintBuilder2D::MaybeAddGlobalConstraint(
  ...
  // node和submap的闭环匹配, 需要保证submap的分支定界匹配器已经构造好了
  constraint_task->AddDependency(scan_matcher->creation_task_handle);
  auto constraint_task_handle =
      thread_pool_->Schedule(std::move(constraint_task));
  finish_node_task_->AddDependency(constraint_task_handle);
...
// creation_task_handle-->constraint_task-->finish_node_task_-->when_done_task_
// 1.creation_task_handle:为subamp构造FastCorrelativeScanMatcher2D
// 2.constraint_task:计算node和submap的闭环约束(主要task)
// 3.finish_node_task_:当前node的所有constraints计算完毕之后, 才会执行一次
// 4.when_done_task_:之前所有nodes的constraints都计算完毕之后, 才会执行一次
//                   调用RunWhenDoneCallback(), 回调Submap2D的HandleWorkQueue()

// NotifyEndOfNode()每个node都会执行一次; WhenDone()每90个nodes才执行一次
// 将finish_node_task_加入到线程池, 并且设置为when_done_task_的依赖
void ConstraintBuilder2D::NotifyEndOfNode() {
  ...
  finish_node_task_ = common::make_unique<common::Task>();
  // when_done_task_ 依赖于 之前所有nodes的constraints计算
  when_done_task_->AddDependency(finish_node_task_handle);
  ++num_started_nodes_;
}

// 绑定when_done_为回调函数:Submap2D::HandleWorkQueue()
// 将when_done_task_加入到线程池, 然后将when_done_task_重置为新的task
void ConstraintBuilder2D::WhenDone(
    const std::function<void(const ConstraintBuilder2D::Result&)>& callback) {
  common::MutexLocker locker(&mutex_);
  // 1.when_done_绑定Submap2D::HandleWorkQueue()
  CHECK(when_done_ == nullptr);
  // TODO(gaschler): Consider using just std::function, it can also be empty.
  when_done_ =
      common::make_unique<std::function<void(const Result&)>>(callback);
  CHECK(when_done_task_ != nullptr);
  // 2.将when_done_task_加入到线程池
  when_done_task_->SetWorkItem([this] { RunWhenDoneCallback(); });
  thread_pool_->Schedule(std::move(when_done_task_));
  // 3.将when_done_task_重置为新的task, 不依赖于之前的任何task
  when_done_task_ = common::make_unique<common::Task>();
}

// 为submap构造FastCorrelativeScanMatcher2D (需要一些时间, 作为task加入线程池)
...
// 计算node和submap的约束(作为task在线程池中运行)
...
// 回调Submap2D的HandleWorkQueue (作为task在线程池中运行)
void ConstraintBuilder2D::RunWhenDoneCallback() {
  ...
  std::unique_ptr<std::function<void(const Result&)>> callback;
  {
    ...
    constraints_.clear(); // 清空之前得到的约束
    callback = std::move(when_done_);
    when_done_.reset(); // 重置when_done_为nullptr
    kQueueLengthMetric->Set(constraints_.size());
  }
  (*callback)(result);
}

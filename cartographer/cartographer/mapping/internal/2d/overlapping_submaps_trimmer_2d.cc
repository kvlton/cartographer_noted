...
class SubmapCoverageGrid2D {
 ...
 private:
  ...
  double resolution_;
  std::map<CellId, StoredType> cells_; // 扫过该cell的submaps
};
...
std::vector<SubmapId> FindSubmapIdsToTrim(
  ...
  // 每个submap覆盖的cell数量 (cell认为只被最新的submap覆盖)
  ...
  for (const auto& cell : coverage_grid.cells()) {
    ...
    for (const std::pair<SubmapId, common::Time>& submap : submaps_per_cell) {
      ++submap_to_covered_cells_count[submap.first]; // 每个submap覆盖的cell数量
    }
  }
  // 当覆盖的cell数量大于min_covered_cells_count时, 保留该submap
  ...
  for (const auto& id_to_cells_count : submap_to_covered_cells_count) {
    ...
    submap_ids_to_keep.push_back(id_to_cells_count.first); // 被保留的submap
  }

  // 两个集合的差集即是将要被删除的submaps集合
...
void OverlappingSubmapsTrimmer2D::Trim(Trimmable* pose_graph) {
  // 1.从pose graph中获取已经finished的submap data
  ...
  // 2.获取第一个submap的范围, 并转化为CoverageGrid,用来做覆盖检测

...
void CastRay(const Eigen::Array2i& begin, const Eigen::Array2i& end,
  ...
  if (begin.x() / kSubpixelScale == end.x() / kSubpixelScale) {
    ...
    for (; current.y() <= end_y; ++current.y()) {
      probability_grid->ApplyLookupTable(current, miss_table); // 查 miss_table_
    }
    return;
  }
...
void CastRays(const sensor::RangeData& range_data,
  ...
  for (const Eigen::Vector3f& hit : range_data.returns) {
    ...
    probability_grid->ApplyLookupTable(ends.back() / kSubpixelScale, hit_table); // 查 hit_table_
  }
  ...
  for (const Eigen::Vector3f& missing_echo : range_data.misses) {
    CastRay(begin, superscaled_limits.GetCellIndex(missing_echo.head<2>()), // 查 miss_table_

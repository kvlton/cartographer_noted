...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  if (insertion_submaps.size() == 1) {
    ...
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
      ...
      optimization_problem_->AddSubmap( // 第一次出现, 则添加
          trajectory_id,
  ...
  // 2.insertion_submaps中有两个submap, 且第二个submap是第一次出现
  ...
  // 3.insertion_submaps中有两个submap, 且都已经出现过了
...
NodeId PoseGraph2D::AddNode(
    ...
    const std::vector<std::shared_ptr<const Submap2D>>& insertion_submaps) {
  // 1.global_pose = local_to_global * local_pose
  const transform::Rigid3d optimized_pose(
      GetLocalToGlobalTransform(trajectory_id) * constant_data->local_pose);

  // 2.NodeId and SubmapId (we never saw before)
  common::MutexLocker locker(&mutex_);
  ...
  // 3.We have to check this here, because it might have changed by the time we
  // execute the lambda.
...
void PoseGraph2D::AddWorkItem(const std::function<void()>& work_item) {
  // 如果work_queue_为nullptr, 直接执行work_item; 否则加入到work_queue_中等待执行
...
// 计算node与submap的constraint (node和submap可能在不同的轨迹上)
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
                                    const SubmapId& submap_id) {
  CHECK(submap_data_.at(submap_id).state == SubmapState::kFinished);

  const common::Time node_time = GetLatestNodeTime(node_id, submap_id);
  // node和submap所在的两条轨迹上一次连接的时间
  const common::Time last_connection_time =
      trajectory_connectivity_state_.LastConnectionTime(
          node_id.trajectory_id, submap_id.trajectory_id);
  // 1.如果node和submap在同一条轨迹上 或者 不在同一条轨迹上但是两条轨迹刚连接上不久
  //   则构造有初始位姿的局部约束
  if (node_id.trajectory_id == submap_id.trajectory_id ||
    ...
    constraint_builder_.MaybeAddConstraint( // 局部约束
        submap_id, submap_data_.at(submap_id).submap.get(), node_id,
        trajectory_nodes_.at(node_id).constant_data.get(),
        initial_relative_pose);
  } else if (global_localization_samplers_[node_id.trajectory_id]->Pulse()) {
    // 2.否则, 构造无初始位姿的全局约束 (相隔很久才计算一次)
    constraint_builder_.MaybeAddGlobalConstraint( // 全局约束
        submap_id, submap_data_.at(submap_id).submap.get(), node_id,
        trajectory_nodes_.at(node_id).constant_data.get());
  }
}

// 计算该submap与其他nodes的constraint
...
void PoseGraph2D::ComputeConstraintsForNode(
  ...
  // 3.为当前node与对应的两个submap构建constraint
  ...
  // 4.计算当前node与其他submaps之间的constraint
  ...
  // 5.若submap刚结束, 则计算该submap与其他nodes的constraint
  ...
  // 6.每90个nodes便会执行一次HandleWorkQueue()进行PoseGraph的优化
...
// 每90个nodes便会执行一次HandleWorkQueue()进行PoseGraph的优化
void PoseGraph2D::DispatchOptimization() {
  run_loop_closure_ = true; // 打断HandleWorkQueue()中work_queue_的执行
  // If there is a 'work_queue_' already, some other thread will take care.
  if (work_queue_ == nullptr) {
    work_queue_ = common::make_unique<std::deque<std::function<void()>>>();
    // 将HandleWorkQueue()传递给ConstraintBuilder2D作为回调函数
...
// 执行PoseGraph的优化, 之后继续执行work_queue_里的任务
void PoseGraph2D::HandleWorkQueue(
    const constraints::ConstraintBuilder2D::Result& result) {
  // 加入新的约束, 再进行PoseGraph的优化
  {
    common::MutexLocker locker(&mutex_);
    constraints_.insert(constraints_.end(), result.begin(), result.end());
  }
  RunOptimization(); // PoseGraph优化
  ...
  // OverlappingSubmapsTrimmer2D 或 PureLocalizationTrimmer
  TrimmingHandle trimming_handle(this);
  for (auto& trimmer : trimmers_) {
    trimmer->Trim(&trimming_handle); // 减少submap和node的数量
  }
  trimmers_.erase(
  ...
  // 继续执行work_queue_, 直到执行完毕 或者 再次请求PoseGraph优化
  run_loop_closure_ = false;
  while (!run_loop_closure_) {
    if (work_queue_->empty()) {
      work_queue_.reset(); // work_queue_执行完毕, 重置为nullptr
      return; // 直接退出
    }
    work_queue_->front()();
...
void PoseGraph2D::RunOptimization() {
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) {
    ...
    // 2.未参与优化的nodes (新的nodes可能还未加入problem)
...
// trajectory_id轨迹在time时刻的pose (node pose)
...
// trajectory_id轨迹从local pose转换到global pose的转移矩阵
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
  ...
  if (begin_it == end_it) {
    ...
    if (it != initial_trajectory_poses_.end()) {
      return GetInterpolatedGlobalTrajectoryPose(it->second.to_trajectory_id,
                                                 it->second.time) *
             it->second.relative_pose; // 手动设置
    } else {
      return transform::Rigid3d::Identity(); // 未手动设置, 设为单位矩阵
    }
  }
  // 2.两者不相等, 表示该轨迹的submap poses不为空
  ...
  return transform::Embed3D(
         ...
         submap_data_.at(last_optimized_submap_id)
             .submap->local_pose()
             .inverse(); // 该轨迹的上一submap的global_pose与local_pose
}

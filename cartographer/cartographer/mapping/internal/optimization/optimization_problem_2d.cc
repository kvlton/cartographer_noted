...
void OptimizationProblem2D::Solve(
  ...
  // 3.Add cost functions for intra- and inter-submap constraints.
  ...
  // 4.Add cost functions for landmarks.
  AddLandmarkCostFunctions(landmark_nodes, freeze_landmarks, node_data_,
                           &C_nodes, &C_landmarks, &problem);

  // 5.Add penalties for violating odometry or changes between consecutive nodes
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (++node_it; node_it != trajectory_end; ++node_it) {
      ...
      // 5.2.Add a relative pose constraint based on the odometry (if available).
      ...
      // 5.3.Add a relative pose constraint based on consecutive local SLAM poses.
  ...
  // 6.Store the result. (global_pose)
